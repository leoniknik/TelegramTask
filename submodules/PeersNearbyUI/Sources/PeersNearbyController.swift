...
public func peersNearbyController(context: AccountContext) -> ViewController {
    ...
    navigateToProfileImpl = { [weak controller] peer, distance in
        if let navigationController = controller?.navigationController as? NavigationController, let controller = context.sharedContext.makePeerInfoController(context: context, titleData: (0, signal: Signal.never()), updatedPresentationData: nil, peer: peer._asPeer(), mode: .nearbyPeer(distance: distance), avatarInitiallyExpanded: peer.largeProfileImage != nil, fromChat: false, requestsContext: nil) {
